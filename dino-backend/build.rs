...
fn main() {
    ...
    let dino_height = image::open("./sprites/dino-run-1.png").unwrap().dimensions().1 as f32;

    writeln!(file, "pub fn obstacle_size(obstacle: &Obstacle) -> (f32, f32) {{").unwrap();
    writeln!(file, "    match obstacle {{").unwrap();
    for (e, image) in enums {
        let image_size = image::open(format!("./sprites/{}", image)).unwrap().dimensions();
        let image_size = (image_size.0 as f32, image_size.1 as f32);
        writeln!(file, "      Obstacle::{} => ({} as f32, {} as f32),", e, image_size.0 / dino_height, image_size.1 / dino_height).unwrap();
    }
    writeln!(file, "    }}").unwrap();
    writeln!(file, "}}").unwrap();
    ...
    writeln!(file, "pub fn random_cactus(rng: &mut ThreadRng) -> Obstacle {{").unwrap();
    writeln!(file, "    match rng.gen_range(0..={}) {{", cactuses.len() - 1).unwrap();
    ...
    writeln!(file, "pub const TALLEST_CACTUS: Obstacle = Obstacle::CactusBig1;").unwrap();
}
